...
function EditPropertyDialog({
  ...
  const [propertyForm, setPropertyForm] = useState(createInitialPropertyForm);
  ...
  const selectedImages = useMemo(
    () => propertyForm.property_images.filter((image) => image instanceof File),
  ...
  const keptExistingCount =
    (existingMainImage && !removeMainImage ? 1 : 0) + keptGalleryCount;

  const finalImageCount = keptExistingCount + selectedImages.length;

  const newImagesBecomeMain = removeMainImage || !existingMainImage;
  ...
  const handleSubmit = async () => {
    ...
    if (finalImageCount === 0) {
      showSnackbar("At least one property image is required.", "warning");
      return;
    }
    ...
    >
      ...
      >
        <PropertyFormFields
          ...
          onToggleRemoveMainImage={handleToggleMainImageRemoval}
          onToggleRemoveExistingGalleryImage={handleToggleExistingGalleryImage}
        />
        ...
        >
          Existing images remain unchanged unless they are marked with ×. Marked
          changes are saved only after Update Property.
        </Alert>
      </DialogContent>
