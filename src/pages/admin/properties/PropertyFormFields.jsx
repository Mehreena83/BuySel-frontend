...
import { clearTypeSpecificFields, filterFieldStyle } from "./propertyFormUtils";
...
function NewImagePreview({ image, index, isMainImage, onRemove }) {
  ...
  return (
    <Box
      sx={{
        ...
        overflow: "hidden",
        borderRadius: "14px",
        border: isMainImage ? "1.5px solid #34d399" : "1px solid #e4e7ec",
        bgcolor: "#ffffff",
...
function PropertyFormFields({
  ...
  const remainingSlots = Math.max(MAX_PROPERTY_IMAGES - totalImageCount, 0);

  const handleChange = (event) => {
    const { name, value } = event.target;

    if (name === "property_type") {
      setPropertyForm((previous) => clearTypeSpecificFields(previous, value));
      return;
    }
  ...
  const handleImageSelection = (event) => {
    ...
    setPropertyForm((previous) => ({
      ...previous,
      property_images: [
        ...previous.property_images.filter((image) => image instanceof File),
        ...acceptedImages,
      ],
    }));
      ...
      >
        ...
        >
          ...
          <Box
            sx={{
              ...
              borderRadius: "10px",
              bgcolor:
                totalImageCount >= MAX_PROPERTY_IMAGES ? "#fef3f2" : "#ecfdf3",
              border:
            ...
            <Box sx={{ mb: 2 }}>
              ...
              >
                ...
                {existingGalleryImages.map((image, index) => (
                  <ExistingImagePreview
                    ...
                    markedForRemoval={removedImageIds.includes(image.id)}
                    onToggleRemove={() =>
                      onToggleRemoveExistingGalleryImage?.(image.id)
                    }
                  />
                ))}
              </Box>
              ...
              {(removeMainImage || removedImageIds.length > 0) && (
                ...
                >
                  ...
                  >
                    Marked images will be removed only after clicking Update
                    Property. Use the green undo icon to restore them.
                  </Typography>
                </Box>
              )}
            </Box>
        ...
        <Box
          ...
          sx={{
            ...
            borderRadius: "14px",
            border: "1.5px dashed",
            borderColor: remainingSlots <= 0 ? "#d0d5dd" : "#86d7b4",
            bgcolor: remainingSlots <= 0 ? "#f8fafc" : "#f7fcfa",
            display: "flex",
            ...
            "&:hover": {
              bgcolor: remainingSlots <= 0 ? "#f8fafc" : "#effaf5",
              borderColor: remainingSlots <= 0 ? "#d0d5dd" : "#34b27b",
            },
          }}
        >
          <Stack
          ...
          >
            <Box
              sx={{
                ...
                flexShrink: 0,
                borderRadius: "13px",
                bgcolor: remainingSlots <= 0 ? "#f2f4f7" : "#dcfce7",
                color: remainingSlots <= 0 ? "#98a2b3" : "#047857",
                display: "grid",
                placeItems: "center",
              }}
            >
            ...
            <Box sx={{ minWidth: 0 }}>
              <Typography
                sx={{
                  color: remainingSlots <= 0 ? "#98a2b3" : "#344054",
                  fontSize: 13.8,
                  fontWeight: 850,
                }}
              >
              ...
              >
                {remainingSlots} slot
                {remainingSlots === 1 ? "" : "s"} available · JPG, PNG or WEBP
              </Typography>
            </Box>
          </Stack>
          ...
          <Box
            sx={{
              ...
              py: 0.9,
              borderRadius: "10px",
              bgcolor: remainingSlots <= 0 ? "#f2f4f7" : "#047857",
              color: remainingSlots <= 0 ? "#98a2b3" : "#ffffff",
              textAlign: "center",
              fontSize: 12.5,
              fontWeight: 850,
              boxShadow:
                remainingSlots <= 0 ? "none" : "0 5px 14px rgba(4,120,87,0.18)",
            }}
          >
            Browse Images
          </Box>
